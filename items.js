...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        let categoryList = { _id: "All", num: 0 };
        this.db.collection('item').aggregate([
            ...
            for (let i = 0; i < docs.length; i++) {
                totalCount+= docs[i].num;
            }
            categoryList.num = totalCount;
            docs.unshift(categoryList);
            callback(docs);
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
		...
		var docToUpdate = { "_id": itemId };
        var updatedDoc = { $push: { "reviews": reviewDoc } };
        
        this.db.collection('item').updateOne(docToUpdate, updatedDoc, function(err, item) {
            assert.equal(err, null);
            callback(item);
        })
